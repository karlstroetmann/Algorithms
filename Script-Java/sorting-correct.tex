...
\item Gesucht ist eine Liste $S$ mit folgenden Eigenschaften: 
  ...
  \item $S$ und $L$ enthalten die selben Elemente, es gilt also \\[0.1cm]
        \hspace*{1.3cm} 
  ...
  \item Die Elemente treten in $L$ und $S$ mit der selben Häufigkeit auf: \\[0.1cm]
        \hspace*{1.3cm} 
        ...
        Forderung, denn wenn alle Elemente in den Listen $L$ und $S$ mit der selben
        Häufigkeit auftreten, dann enthalten $L$ und $S$ natürlich auch die selben Elemente.
  \end{enumerate}
\end{enumerate}
...
Effizienz aber zu wünschen übrig lässt.  Im Anschluss daran präsentieren wir zwei
...
\begin{Satz}[Distributivität von \texttt{count} über \texttt{sort}] \hspace*{\fill}\\
{\em
...
\item Andernfalls suchen wir in der Liste $L$ das kleinste Element und entfernen dieses
      ...
      weniger enthält.  Zum Schluss fügen wir das kleinste Element vorne an die sortierte
      Liste an: \\[0.1cm]
      \hspace*{1.3cm} 
...
\begin{Satz}[Distributivität von \texttt{count} über \texttt{sort}] \hspace*{\fill} \\[0.1cm]
{\em
...
zu einer Funktion \texttt{split} zusammen gefasst, die zwei Listen zurück liefert. Ein
Aufruf der Form
\\[0.2cm]
\hspace*{1.3cm}
...
\item Anschließend teilen wir das, was jetzt noch von der Liste \texttt{list} übrig ist,
      ...
      \texttt{second} auf.
\item Zum Abschluss fügen wir das Element $x$ in die Liste \texttt{first} ein
...
\begin{Satz}[Verträglichkeit von \texttt{isSorted} und \texttt{sort}] \hspace*{\fill} \\
{\em
...
\begin{Satz}[Verträglichkeit von \texttt{count} und \texttt{sort}] \hspace*{\fill} \\
{\em
...
ist bei der Implementierung von ``\emph{Sortieren durch Mischen}'' ein Hilfs-Feld erforderlich, das die selbe Größe hat
