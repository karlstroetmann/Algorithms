...
{\sl
Ein Bauer will mit einem Wolf, einer Ziege und einem Kohl über einen Fluss übersetzen, um
...
Wenn er den Wolf mit der Ziege allein lässt, dann frisst der Wolf die Ziege und wenn er die
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{Lösung des Wolf-Ziege-Kohl-Problems in \textsc{Setl2}.}
...
Rot-Schwarz-Bäumen implementiert, während die Klasse \texttt{HashSet} mit Hilfe von
Hash-Tabellen implementiert ist.  Letztere haben im schlechtesten Fall eine höhere
Komplexität.  Daher entscheiden wir uns für die Klasse \texttt{TreeSet}.  Versuchen wir
das Programm aus Abbildung \ref{fig:wolf-ziege-kohl.stl} in \textsl{Java} umzusetzen,
so stoßen wir allerdings sofort auf ein Problem:  Bei der Umsetzung benötigen wir Mengen,
...
Nachdem wir nun mit Mengen so arbeiten können, wie wir dass in dr Sprache \textsl{Setl} gewohnt sind,
...
dieselben Funktionen wie in \textsl{Setl} zur Verfügung stehen, können wir nun daran
gehen, das im ersten Semester entwickelte \textsl{Setl}-Programm in \textsl{Java} zu übersetzen.
