...
the number of times that the test \texttt{y$_n$ > 0} of \texttt{while} loop has been executed.
After the loop in line 10 the variable is indexed as
$\texttt{r}_N$, where $N$ denotes the total number of times that the test \texttt{y$_n$ > 0} has been executed.
...
\begin{enumerate}
\item[B.C.] $n=1$: Since we have $r_1 = 1$, $x_1 = a$, and $y_1 = b$ we have 
            \\[0.2cm]
            \hspace*{1.3cm}
