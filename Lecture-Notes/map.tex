...
\section{Ordered Ninary Trees}
If the set \textsl{Key} is linearly, i.e.~if there exists a binary relation
...
\end{enumerate}
In order to define ordered binary trees we introduce
...
\begin{Definition}[Binary Trees] \hspace*{\fill} \\
  ...
  \item $\textsl{Node}(k, v, l, r)$ is a binary tree if the following holds true: 
        ...
        \item $l$ is a binary tree.
              ...
              $l$ is the  \emph{left subtree} of the tree $\textsl{Node}(k, v, l, r)$.
        \item $r$ is a binary tree.

              $r$ is the \emph{right subtree} of the tree $\textsl{Node}(k, v, l, r)$.
              \eox
        \end{enumerate}
  \end{enumerate}
\end{Definition}
...
\noindent
Next, we define the notion of an  \emph{ordered binary tree}.
\begin{Definition}[Ordered Binary Tree] \hspace*{\fill} \\
  ...
  \item $\textsl{Node}(k, v, l, r) \in \Bin_<$ \quad iff the following conditions hold:
        ...
        \end{enumerate}
        The last two conditions are known as the \emph{ordering conditions}.
        \eox
  \end{enumerate}
\end{Definition}
...
\item A binary tree of the form $\textsl{Node}(k,v,l,r)$ is represented by an oval.  Inside of this
      ...
      bot are separated by a horizontal line.  This oval is then called a \emph{node} of the binary tree.
...
node that has the key $8$ and the value $22$ is called the \emph{root} of the binary tree.
A \emph{path of length} $k$ in the tree is  list $[n_0,n_1, \cdots, n_k]$ of
...
Things get more difficult when neither  $l$ nor $r$ are empty.  In this case,
...
\item $l\not= \textsl{Nil} \wedge l.\textsl{delMin}() = [l',k_{min}, v_{min}] \;\rightarrow$ \\[0.1cm]
      ...
      is not empty, than the smallest key of  $t$ is located inside the left subtree $l$.
...
\end{Verbatim}
\vspace*{-0.3cm}
  \caption{Implementierung geordneter B\"aume in \textsc{SetlX}, 1.~Teil.}
