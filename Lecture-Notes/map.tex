...
following, functional relations will be called \blue{maps}.  In \textsl{Python}, maps are known as
\blue{dictionaries}.  Maps are also known as
\href{https://en.wikipedia.org/wiki/Associative_array}{\blue{associative arrays}},  or \blue{symbol tables}.
...
maps can be implemented efficiently.  We confine our attention to the implementation of maps.  Once we know how
to implement a map, it is easy to see that a set $S$ can be represented by a function $f_S$ that satisfies
\\[0.2cm]
\hspace*{1.3cm}
...
\item Next, we discuss \blue{balanced ordered trees}.  In the case of balanced ordered trees the
      ...
      In particular we discuss \blue{AVL trees} and \blue{2-3 trees}.
...
\begin{Definition}[Ordered Binary Trees] \hspace*{\fill} \\
  Assume a set $\textsl{Key}$ and a set $\textsl{Value}$ are given.
  The set $\Bin$ of \blue{ordered binary trees}\index{ordered binary trees} is defined inductively as the set
  of \blue{terms} that are constructed from the function symbols \texttt{Nil} and \texttt{Node}, where the
  signatures of these function symbols are given as follows: \\[0.2cm]
  \hspace*{1.3cm} 
  ...
  \item $\texttt{Node}(k, v, l, r) \in \Bin$ \quad iff the following conditions hold:
        ...
        \item $l$ is an ordered binary tree.
              ...
              $l$ is the  \blue{left subtree} of the tree $\texttt{Node}(k, v, l, r)$.
        \item $r$ is a ordered binary tree.

              $r$ is the \blue{right subtree} of the tree $\texttt{Node}(k, v, l, r)$.
...
\begin{enumerate}
\item The empty tree \texttt{Nil} is either shown as a black dot or it is not shown at all.
...
trees can be implemented in \textsl{Python}.  Objects of class \texttt{map} represent ordered
...
\item The class \texttt{OrderedBinaryTree} represents a node in an ordered binary tree.  In order to do so, it
      ...
      \end{enumerate}
\item The function \texttt{isEmpty} checks whether \texttt{self} represents an empty tree.
...
\item The implementation of \texttt{insert} is similar to the implementation of \texttt{find}.
      ...
      \item If the key \texttt{key}, for which the value \texttt{value} is to be inserted, is identical
            to the key \texttt{mKey} stored at this node, then we have found the node where we need
            to insert \texttt{value}.  In this case, the current value of the variables \texttt{mValue} is
            overwritten with \texttt{value}. 
      \item Otherwise, \texttt{key} is compared with \texttt{mKey} and the insertion is recursively continued in the
            appropriate subtree.
      \end{enumerate}      
...
\item The implementation of \texttt{delMin} and \texttt{delete} is done in a similar way as the
      ...
      \texttt{self = mLeft}
      \\[0.2cm]
      However, we cannot replace the object \texttt{self} with another object.  The only thing we can do is
      change the attributes of the object \texttt{self}.  This is done in the method \texttt{update}.
      This method overwrites the member variables of \texttt{self} with the corresponding member variables of
      \texttt{t}. 
\end{enumerate}
...
binary search tree containing $n$ key-value pairs.  After that, we investigate the average case complexity.  We
