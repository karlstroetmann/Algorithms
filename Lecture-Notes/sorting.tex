...
\begin{figure}[!ht]
  \centering
  \framebox{\epsfig{file=../Python/Chapter-04/lomuto.png, scale=0.40}} 
...
that only give $\mathtt{True}$ or $\mathtt{False}$ answers.  Tests of this kind are called \blue{binary tests}.
\begin{enumerate}
...
\\[0.2cm]
\hspace*{1.3cm}
$k \geq n \cdot \ds\log_2(n) - \frac{n}{\,\ln(2)\,}$.
\\[0.2cm]
...
\begin{minted}[ frame         = lines, 
    ...
    def countingSort(Students):
        ...
        # Phase 2: Indexing
        Index[0] = 0
        for grade in range(1, maxGrade+1):
            Index[grade] = Index[grade-1] + Counts[grade-1]
...
\item The procedure \texttt{countingSort} receives one argument.  The list $\texttt{Students}$
      ...
      The purpose of the function \texttt{countingSort} is to sort this list w.r.t.~the grades of the students.
      Sublists of students with the same grade should still be sorted alphabetically w.r.t.~the names.
...
\item The list \texttt{Sorted} is the list that will be returned as the result.
      ...
      and \texttt{grade} is the grade of this student.
\item The \texttt{for}-loop in line 7--8 performs the \blue{counting stage}.  We iterate over all grades
...
\item Then, the \texttt{for}-loop in line 11-12 performs the \blue{indexing stage}. As the number
      $\texttt{Index}[\mathtt{grade}-1]$ is the index of the start of the sublist for those students that have
      achieved the grade  $\mathtt{grade}-1$ and the number of these
      students is $\texttt{Counts}[\mathtt{grade}-1]$, the sublist of the students with grade $\mathtt{grade}$ has to start at index
      \\[0.2cm]
      \hspace*{1.3cm}
      $\texttt{Index}[\mathtt{grade}-1] + \texttt{Counts}[\mathtt{grade}-1]$.
\item Finally, the \texttt{for}-loop in line 14--17 performs the \blue{distribution stage}.
      \begin{enumerate}
...
\item Analyse the average complexity of your algorithm. \eox
\end{enumerate}   
\pagebreak
