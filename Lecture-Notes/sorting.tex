...
\blue{merge sort} and \blue{quicksort}.  In the average case, both of these algorithms are very efficient when implemented carefully.
...
\begin{Definition}[Partial Order] \index{partial order}
  ...
  \begin{enumerate}
  \item $\forall x \el S \colon x \leq x$,                      \hspace*{\fill} ($\leq$ is \blue{reflexive})\index{reflexive}
  \item $\forall x, y \el S \colon \bigl(x \leq y \wedge y \leq x
  \rightarrow x = y\bigr)$,   \hspace*{\fill} ($\leq$ is \blue{anti-symmetric})\index{anti-symmetric}
  \item $\forall x, y, z \el S \colon \bigl(x \leq y \wedge y \leq z \rightarrow x \leq z\bigr)$.  \hspace*{\fill} ($\leq$ is \blue{transitive})\index{transitive}
  \end{enumerate}
  The relation $\leq$ is then called a \blue{partial order}. \eox
\end{Definition}
...
\begin{Definition}[Linear Order\label{def:linear_order}] \index{linear order}
  ...
  \\[0.2cm]
  The relation $\leq$ is then called a \blue{linear order} or a \blue{total order}.\index{total order} \eox
  \eox
\end{Definition}
...
\begin{Definition}[Quasiorder] \index{quasiorder} \hspace*{\fill} \\
{\em
  A pair $\langle S, \preceq\rangle$ is a \href{https://en.wikipedia.org/wiki/Preorder}{quasiordered set} \index{quasiorder}  iff $\preceq$ is a 
  ...
  \end{enumerate}
  The relation $\preceq$ is then called a \blue{quasiorder}.  If, furthermore,
  \\[0.2cm]
  \hspace*{1.3cm}
  $\forall x, y \el S \colon \bigl(x \preceq y \vee y \preceq x\bigr)$ \hspace*{\fill} (linearity) \index{linearity}
  \\[0.2cm]
  holds, then $\langle S, \preceq \rangle$ is called a \blue{total quasiordered set} and $\preceq$ is then
  called a \blue{total quasiorder}.  This notion will be abbreviated as \textsc{TQO}.\index{TQO}
}
\end{Definition}
...
\subsection{Complexity of Insertion Sort: Worst Case}
Next, we will compute the number of comparisons that are done in the implementation of \mytt{insert}
...
In order to sort a list $\mytt{L}$ the algorithm proceeds as follows:
\begin{enumerate}
...
\item Otherwise, the list $\mytt{L}$ is split into two lists that have approximately the same size.
     ...
     $
      ...
      first part.  The function \mytt{merge} takes two sorted lists and combines their elements in a way such that
...
Next, we need to specify how two sorted lists $\mytt{L}_1$ and $\mytt{L}_2$ are merged in a way that the resulting list
is sorted.
\begin{enumerate}
...
\blue{merge sort} with \blue{quicksort} later, the result $f(n) \in \Oh\bigl(n \cdot \log_2(n) \bigr)$ is
...
\href{http://en.wikipedia.org/wiki/Quicksort}{quicksort} algorithm \cite{hoare:61}.  \index{quicksort}
...
\item Otherwise, $\mytt{L}$ has the form $\mytt{L} = [\mytt{x}] + \mytt{R}$.  In this case, we split $\mytt{R}$ into two lists $\mytt{S}$ and $\mytt{B}$.
      ...
      is called \blue{partitioning}.\index{partitioning}  The element \mytt{x} which is used to partition
      the list is called the \blue{pivot element}.\index{pivot element}
...
list $\mytt{S}$ that are smaller than or equal to the pivot element $\mytt{x}$ is at least $0$ and at most $n$
and hence it is a member of the set 
$\{0,1,2,\cdots,n\}$.  If the length of $\mytt{S}$ is $i$ and the length of $\mytt{L}$ is $n+1$, then the length
of the list $\mytt{B}$ of those elements that are bigger than $\mytt{x}$ is $n-i$.  Therefore, if
$\mytt{len}(\mytt{S}) = i$, then we need on average 
\\[0.2cm]
...
\\[0.2cm] 
Here, the term $n$ accounts for the number of comparisons needed to partition the list $\mytt{R}$ into \mytt{S}
and \mytt{B}.
...
\begin{array}[t]{lclcl}
 \ds
 \sum\limits_{i=0}^n a_{n-i} & = & a_n + a_{n-1} + \cdots + a_1 + a_0                                  \\[-0.5cm]
                    & = & a_0 + a_1 + \cdots + a_{n-1} + a_n        & = & \ds \sum\limits_{i=0}^n a_i 
\end{array}
$
\\[0.2cm]
holds for any sequence of numbers $(a_n)_{n\in\mathbb{N}}$.
...
\\[0.2cm]
In order to get rid of the sum, we multiply equation (3) with $n$ and equation (2) with $n+1$.  This yields the
equations 
\\[0.2cm]
\hspace*{1.3cm}
...
\\[0.2cm] 
In order to exhibit the underlying structure of this equation we divide it by  $(n+1) \cdot(n+2)$ and
...
\index{harmonic number, $H_n = \sum\limits_{i=1}^n\frac{1}{i}$}
...
comparisons and hence on average \blue{quicksort} needs  $2 \cdot \ln(2) \approx 1.39$ times more comparisons than
\blue{merge sort}.  
...
devised by Nico Lomuto \cite{cormen:09}. \index{Lomuto, Nico}  This scheme is less efficient than the original
...
\end{minted}
\vspace*{-0.7cm}
  \caption{The implementation of the function \blue{partition}.}
  \label{fig:quick-sort-array.stlx-partition}
\end{figure}
\FloatBarrier

\noindent
The difficult part of the implementation of \blue{quicksort} is the implementation of the
...
implementation of the function \mytt{swap} that swaps the elements in the list \mytt{L} at the indices \mytt{x}
and \mytt{y}. 
...
\end{minted}
\vspace*{-0.7cm}
  \caption{The implementation of the function \mytt{swap}.}
...
\item In 2009, Vladimir Yaroslavskiy introduced \blue{dual pivot quicksort}
      ...
      \textsl{Java} uses \blue{dual pivot quicksort}: 
      \\[0.2cm]
      \hspace*{-0.6cm}
...
\\[0.2cm]
different \blue{permutations} of the list $L$, where a \blue{permutation} \index{permutation} of $L$ is any
list that contains the same elements as the list $L$, but possibly in a different order.
The expression $n!$ is called the
\href{https://en.wikipedia.org/wiki/Factorial}{factorial} of $n$. \index{factorial}
\end{Theorem}
...
\begin{array}[t]{lcll}
  k & \geq & \ds\log_2(n!) \\[0.2cm]
    ...
    & =    & \ds\sum\limits_{i=2}^n \log_2(i) & \mbox{(since $\log_2(1) = 0$)} \\[0.5cm]
...
iterates $n$ times.  Hence, counting sort is a \blue{linear sorting algorithm}.  Note that we do not compare different grades in
order to sort them.
...
\item The function $\mytt{distance}(x, y)$ computes the Euclidean distance between the images $x$ and $y$.
      ...
      components of $x$.  We compute the sum of these squares using the \mytt{numpy} function \mytt{sum}.
...
\begin{enumerate}[(a)]
\item Devise an algorithm \blue{QuickSelect} that selects the $k$ smallest elements from a list $L$.
...
\begin{enumerate}
\item Define the concept of a \blue{total quasi order}.
...
\item Compare the memory requirements of \blue{merge sort} and \blue{quicksort}.
