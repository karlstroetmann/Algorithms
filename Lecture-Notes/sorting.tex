...
efficiency of this algorithms is far from optimal.  Next, we present \blue{quicksort} and 
\blue{merge sort}.  In the average case, both of these algorithms are very efficient when implemented carefully.
...
\begin{Definition}[Linear Order\label{def:linear_order}] \index{linear order}
  ...
  \\[0.2cm]
  The relation $\leq$ is then called a \blue{linear order}. \eox
  \eox
\end{Definition}
...
{\em
  ...
  called a \blue{total quasiorder}.  This notion will be abbreviated as \textsc{TQO}.
}
\end{Definition}
...
\item Otherwise, the list $L$ must have the form $[x] + R$. Here, $x$ is the first element of $L$
      and $R$ is the rest of $L$, that is $R$ contains all elements of $L$ but the first element.  In order to sort
...
$\mytt{L}$ of length $n$.  The reason is this:  If $\mytt{x}$ is bigger than any element of $L$, then we have to
...
\begin{enumerate}
\item If $L$ has less than two elements, then $L$ is already sorted.  Therefore we have: what
      \\[0.2cm]
      \hspace*{0.3cm}
...
\item Otherwise, the list $L$ is split into two lists that have approximately the same size.
      ...
      \hspace*{0.3cm} 
      $n := \mytt{len}(L) \wedge n \geq 2 \rightarrow \mytt{sort}(L) =
     ...
     $
      ...
      $\mytt{L[n\dv 2:]}$ is the second part that contains the remaining elements.  If the length of $L$ is
      even, both part have the same number of elements, otherwise the second part has one element more than the
      first part.  The function \mytt{merge} takes two sorted lists and combines their elements in a way that
