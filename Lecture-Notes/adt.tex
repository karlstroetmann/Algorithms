...
\href{https://en.wikipedia.org/wiki/CLU_(programming_language)}{\textsc{Clu}}. \index{Clu}
The other ingredient of object oriented programming is \blue{inheritance}.  However, the notion of inheritance
...
\item $\textsl{Ts}$ is a set of \blue{type specifications}.  For every function symbol
      ...
      these data types: 
      \begin{enumerate}
      \item We can have predefined data types like, e.g.~``\mytt{int}'' or ``\mytt{str}''.
...
\mytt{Stack}\index{Stack} as follows: 
\begin{enumerate}
\item The name of this abstract data type is \mytt{Stack}.
...
\item The type specifications for these function symbols are given as follows:
      ...
      \item $\mytt{stack}: \mytt{Stack}$
            ...
            Therefore, this function is a \blue{constructor}.  Intuitively, the function call $\mytt{stack}()$ 
...
\item $\mytt{stack}().\mytt{top}() = \Omega$
      ...
      In \textsl{Python}, $\Omega$ is represented as \mytt{None}. The expression $\mytt{stack}()$
...
functions \mytt{stack} and \mytt{push} as \blue{generators},\index{generator}  then the axioms specify the
...
discussed next.
\begin{enumerate}
\item The definition of the \textsc{Adt} \textsl{Stack} starts with the keyword \mytt{class}
      in line 1.
      After the keyword \mytt{class}, the name of the class has to be given.  In Figure
      \ref{fig:stack-array.stlx} this name is \mytt{Stack}.
\item In \textsl{Python}, the \blue{constructor} of a class has the name \mytt{\_\_init\_\_}.
      All methods defined in a class receive the object as their first argument.
      The convention is to name this parameter \mytt{self}, but this is not mandatory.
      The name \mytt{self} is similar to the keyword \mytt{this} in the programming language
      \textsl{Java}.  However, technically the name \mytt{self} is not a keyword in \textsl{Python}.
      
      The constructor receives an \blue{uninitialized} object as its first argument and has the task
      to initialize the \blue{member variables} of this object.  The class \mytt{Stack} uses only one member variable:
      This member variable is called \mytt{mStackElements}.  My convention is to always start member
      variables with the letter '\mytt{m}'.  Another convention is to use the underscore character
      '\mytt{\_}'.  These conventions facilitates the distinction of member variables from local variables.

      In order to create an object of class \mytt{Stack} we invoke the constructor as follows:
      \\[0.2cm]
      \hspace*{1.3cm}
      \mytt{s = Stack()}
      \\[0.2cm]
      This statement creates an uninitialized object (in other words: an empty object) of class \mytt{Stack}
      and then invokes the constructor \mytt{\_\_init\_\_} to initialize the member variable
      \mytt{mStackElements} as an empty list.  Next, object created is assigned to the variable \mytt{s}.
\item Line 3 defines the first (and in this case only) member variable of the class \mytt{Stack}.
      Therefore, every object $o$ of class \mytt{Stack} will have a \blue{member variable} \index{member variable}
      called
      \mytt{mStackElements}. 
      We will use this list to store the elements of the stack.  To retrieve the member variable
      \mytt{mStackElements} from an object $o$ we use the following expression:
      \\[0.2cm]
      \hspace*{1.3cm}
      $o$\mytt{.mStackElements}
      \\[0.2cm]
...
\item The rest of the class definition contains a number of function definitions.  Functions defined inside
      ...
      \mytt{Stack}.  For example, 
      \\[0.2cm]
      \hspace*{1.3cm}
      \mytt{stack.push}
      \\[0.2cm]
      refers to the method \mytt{push} defined in line 5 and 6. Every object of class
      \mytt{Stack} has access to these 
      methods.  For example, if $s$ is an object of class \mytt{Stack}, then we can invoke the
      method \mytt{push} by writing:
      \\[0.2cm]
      \hspace*{1.3cm}
      $s$\mytt{.push($x$)}
\item Line 5 starts the definition of the method \mytt{push}.  This method is called with two arguments:
      \begin{enumerate}[(a)]
      \item \mytt{self} refers to the \mytt{Stack} object.
      \item $e$ is the element that is to be pushed on the stack.  In the array based
            implementation, this is achieved by appending $e$ to the list \mytt{mStackElements}.
      \end{enumerate} 
      When invoking the method \mytt{push}, we have to specify the stack by prefixing it to the method
      invocation.  That is, if $s$ is a stack and we want to push $e$ onto this stack, then we can do this by
      writing: 
      \\[0.2cm]
      \hspace*{1.3cm}
      $s\mytt{.push}(e)$
\item Line 8 starts the implementation of the method \mytt{pop}, which has the task to remove 
      one element from the stack.  Of course, it would not make sense to remove an element from the
      stack if the stack is empty.  Therefore, the \mytt{assert} statement in line 9 checks
      whether the number of elements of the list \mytt{mStackElements} is bigger than $0$.
      If this condition is satisfied, the last element of the list \mytt{mStackElements} is removed.
\item Line 12 starts the definition of the method \mytt{top}.  First, it is checked that the stack
      is non-empty.  Then, the element at the end of the list \mytt{mStackElements} is returned.
\item Line 16 defines the method \mytt{isEmpty}.  This method checks whether the list
      \mytt{mStackElements} is empty.
\item Line 19 defines the method \mytt{copy}.  The purpose of this method is to create an exact
      copy of the given stack.  To this end the method creates a new object $C$ of class \mytt{Stack}.
      Then the member variable \mytt{mStackElements} of the object \mytt{self} that was used to invoke
      the method \mytt{copy} is copied into the member variable \mytt{mStackElements} of the object $C$.
...
\item Line 24 defines the method \mytt{\_\_str\_\_}.  This method serves a similar purpose as the method
      \mytt{toString} in a \textsl{Java} program:  If an object of class \mytt{Stack} needs to
      be converted into a string, then the method \mytt{\_\_str\_\_} is invoked automatically to
      perform this conversion.

      In order to understand the implementation of \mytt{\_\_str\_\_} we execute the following statements:
      \\[0.2cm]
      \hspace*{1.3cm}
      \mytt{s = stack(); s.push(1); s.push(2); s.push(3); print(s)}
      \\[0.2cm]
      ...
      is the result of calling \mytt{\_\_str\_\_} and has the following form:
      \begin{verbatim}
      -------------
      | 1 | 2 | 3 |
      -------------
      ...
      The implementation of the method \mytt{\_\_str\_\_} works as follows.
      \begin{enumerate}
      \item First, we use the auxiliary method \mytt{\_convert}.  This method computes a string of
            the form \\[0.2cm]
            \hspace*{1.3cm} \mytt{| 1 | 2 | 3 |}. 
            \\[0.2cm]
            The implementation of \mytt{\_convert} is done via a case distinction:
            If the given stack is empty, the result of \mytt{\_convert} will be the string ``\mytt{|}''.  
            Otherwise we get the top element $t$ of the stack using the method \mytt{top()} and remove
            it using \mytt{pop()}.  Next, the remaining stack is converted to a string 
            recursively and finally the element $t$ is appended to this string.

            The name of the method \mytt{\_convert} starts with an underscore because
            \mytt{\_convert} is a \blue{private} method of the class \mytt{Stack}, i.e.~it should not be
            used from outside of the class \mytt{Stack}: Only methods defined in the class \mytt{Stack} are
            permitted to use the method \mytt{\_convert}.  However, this restriction is not enforced by the
            \textsl{Python} interpreter.
      \item The method \mytt{\_\_str\_\_} creates a line of dashes in line 27.
            This line has the same length as the string produced by \mytt{\_convert}.
            The result of \mytt{\_convert} is then decorated with these dashes.
      \end{enumerate}
\item The function $\mytt{createStack}(L)$ converts a list $L$ into a stack and returns the resulting
      \mytt{Stack} object.
\end{enumerate}
You should note that we were able to implement  the method \mytt{\_\_str\_\_} without knowing anything
about the internal representation of the stack.  In order to implement \mytt{\_\_str\_\_} we only used
the methods \mytt{top}, \mytt{pop}, and \mytt{isEmpty}.  This is one of the main advantages of
...
``\mytt{\symbol{37}}'', and ``\mytt{**}''. Here, the operator ``\mytt{/}'' denotes integer division,
...
\item If $s$ and $t$ are arithmetic expressions, then the string
      ...
      denote string concatenation, while \mytt{'*'} denotes the string consisting of the multiplication
      operator. 
      ...
      \\[0.2cm]
      are arithmetic expressions.  We interpret  $s + \mytt{'/'} + t$ as the \blue{integer division} of $s$
      by $t$.
...
In mathematics the operators ``\mytt{*}'', ``\mytt{/}'' and ``\mytt{\symbol{37}}'' have a
...
``\mytt{+}'', ``\mytt{-}'', ``\mytt{*}'', ``\mytt{/}'', and ``\mytt{\symbol{37}}''
\blue{associate to the left}:  An expression of the form 
\\[0.2cm]
\hspace*{1.3cm} 
\mytt{1 - 2 - 3} \quad is interpreted as \quad \mytt{(1 - 2) - 3}.
 \\[0.2cm]
Finally, the operator ``\mytt{**}'' \blue{associates to the right}:
The arithmetic expression \\[0.2cm]
\hspace*{1.3cm} 
\mytt{2 \mytt{**} 3 \mytt{**}  2} \quad is interpreted as \quad 
\mytt{2 \mytt{**} (3 \mytt{**} 2)}. 
\\[0.2cm]
...
\\[0.2cm]
\hspace*{1.3cm}
\mytt{[1, \symbol{34}+\symbol{34}, 2, \symbol{34}*\symbol{34}, 3, \symbol{34}-\symbol{34}, 4]}.
\\[0.2cm]
...
\item In the beginning, the token stack contains the tokens of the arithmetic expression and the other two stacks
      ...
      \hspace*{1.3cm} \mytt{mArguments = []}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mOperators = []}. 
\item The number \mytt{1} is removed from the token stack and is put onto the argument stack
      instead.  The three stacks are now as follows: \\[0.2cm]
      \hspace*{1.3cm} \mytt{mTokens \ \ \ = [ 4, \symbol{34}-\symbol{34}, 3, \symbol{34}*\symbol{34}, 2, \symbol{34}+\symbol{34} ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mArguments = [ 1 ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mOperators = []}. 
\item Next, the operator \mytt{\symbol{34}+\symbol{34}} is removed from the token stack and is put
      onto the operator stack.  Then we have: \\[0.2cm]
      \hspace*{1.3cm} \mytt{mTokens \ \ \ = [ 4, \symbol{34}-\symbol{34}, 3, \symbol{34}*\symbol{34}, 2 ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mArguments = [ 1 ]} \\[0.2cm]
      \hspace*{1.3cm} \mytt{mOperators = [ \symbol{34}+\symbol{34} ]}. 
\item Now, we remove the number \mytt{2} from the  token stack and put it onto the argument stack.
      We have: \\[0.2cm]
      \hspace*{1.3cm} \mytt{mTokens \ \ \ = [ 4, \symbol{34}-\symbol{34}, 3, \symbol{34}*\symbol{34} ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mArguments = [ 1, 2 ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mOperators = [ \symbol{34}+\symbol{34} ]}. 
\item We remove the operator \mytt{\symbol{34}*\symbol{34}} from the  token stack and compare the
      \blue{precedence} of the operator \mytt{\symbol{34}*\symbol{34}} with the precedence of the operator
      \mytt{\symbol{34}+\symbol{34}}, which is on top of the operator stack.  Since the precedence of the operator 
      \mytt{\symbol{34}*\symbol{34}} is greater than the precedence of the operator 
      \mytt{\symbol{34}+\symbol{34}}, the operator \mytt{\symbol{34}*\symbol{34}} is put onto
      the operator stack.  The reason is that we have to evaluate this operator before we can
      evaluate the operator \mytt{\symbol{34}+\symbol{34}}.  Then we have: 
      \\[0.2cm]
      \hspace*{1.3cm} \mytt{mTokens \ \ \ = [ 4, \symbol{34}-\symbol{34}, 3 ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mArguments = [ 1, 2 ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mOperators = [ \symbol{34}+\symbol{34}, \symbol{34}*\symbol{34}]}. 
\item We remove the number \mytt{3} from the  token stack and put it onto the argument stack.
      \\[0.2cm]
      \hspace*{1.3cm} \mytt{mTokens \ \ \ = [ 4, \symbol{34}-\symbol{34} ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mArguments = [ 1, 2, 3 ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mOperators = [ \symbol{34}+\symbol{34}, \symbol{34}*\symbol{34} ]}. 
\item We remove the operator \mytt{\symbol{34}-\symbol{34}} from the token stack and
      compare this operator with the operator \mytt{\symbol{34}*\symbol{34}}, which is on top of
      the operator stack.  As the precedence of the  operator \mytt{\symbol{34}*\symbol{34}} is
      higher than the precedence of the operator \mytt{\symbol{34}-\symbol{34}},
      we have to evaluate the operator \mytt{\symbol{34}*\symbol{34}}.  In order to do so, we
      remove the arguments 3 and 2 from the argument stack, remove the operator
      \mytt{\symbol{34}*\symbol{34}} from the operator stack and compute the product of the two
      arguments.  This product is then put back on the 
      argument stack.  The operator \mytt{\symbol{34}-\symbol{34}} is put back on the token stack
      since it has not been used.  Hence, the stacks look as shown below: \\[0.2cm]
      \hspace*{1.3cm} \mytt{mTokens \ \ \ = [ 4, \symbol{34}-\symbol{34} ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mArguments = [ 1, 6 ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mOperators = [ \symbol{34}+\symbol{34} ]}. 
\item Again, we take the operator \mytt{\symbol{34}-\symbol{34}} from the token stack and
      compare it with the operator \mytt{\symbol{34}+\symbol{34}} that is now on top of the
      operator stack.  Since both operators have the same precedence, the operator
      \mytt{\symbol{34}+\symbol{34}} is evaluated:  We remove two arguments from the argument
      stack, remove the operator
      \mytt{\symbol{34}+\symbol{34}} from the operator stack  and compute the sum of the
      arguments.  The result is put back on the argument stack.  Furthermore, the operator
      \mytt{\symbol{34}-\symbol{34}} is put back on the token stack.
      Then we have: \\[0.2cm]
      \hspace*{1.3cm} \mytt{mTokens \ \ \ = [ 4, \symbol{34}-\symbol{34} ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mArguments = [ 7 ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mOperators = []}. 
\item Next, the operator \mytt{\symbol{34}-\symbol{34}} is removed from the token stack and is now
      put on the operator stack.  We have: \\[0.2cm]
      \hspace*{1.3cm} \mytt{mTokens \ \ \ = [ 4 ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mArguments = [ 7 ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mOperators = [ \symbol{34}-\symbol{34} ]}. 
\item The number \mytt{4} is removed from the token stack and put onto the argument stack. We have: \\[0.2cm]
      \hspace*{1.3cm} \mytt{mTokens \ \ \ = []}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mArguments = [ 7, 4 ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mOperators = [ \symbol{34}-\symbol{34} ]}. 
\item Now the input has been consumed completely.
      Hence, the operator \mytt{\symbol{34}-\symbol{34}} is removed from the  operator stack and
      furthermore, the arguments of this operator are removed from the argument stack.  Then, the
      operator \mytt{\symbol{34}-\symbol{34}} is evaluated and the result is put onto the argument
      stack.  We have: \\[0.2cm]
      \hspace*{1.3cm} \mytt{mTokens \ \ \ = []}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mArguments = [ 3 ]}, \\[0.2cm]
      \hspace*{1.3cm} \mytt{mOperators = []}. \\[0.2cm]
      Therefore, the result of evaluating the arithmetic expression ``\mytt{1+2*3-4}'' is the
      number 3.
\end{enumerate}

\subsection{The Algorithm \label{algo-arith}}
...
(1930-2002) in 1961. \index{Dijkstra, Edsger}  The algorithm uses three stacks:
\begin{enumerate}
\item The \blue{token stack} contains the tokens that are to be parsed.  For example,
...
\item The \blue{operator stack} contains the operators that have to be evaluated.
\item The \blue{argument stack} contains the arguments for these operators.
\end{enumerate}
...
\item If the next token is a closing parenthesis but the operator on top of the operator stack is not an
      ...
      In the remaining case neither the token on top of the token stack nor the operator on top of the
      operator stack can be a parenthesis.  
\item If the operator on top of the operator stack needs to be evaluated before the operator on top of the token stack,
      the operator on top of the operator stack is evaluated.  Otherwise, the operator on top of the token
      stack is pushed onto the operator stack.
\item Once the token stack is empty, the remaining operators of the operator stack are evaluated one by one.    
\end{enumerate}
The algorithm finishes when both the token stack and the operator stack are empty.  If the arithmetical
expression that is evaluated is syntactically correct, there should be exactly one number left on the
argument stack.  This number is the result of the evaluation.
      
We give a more detailed representation of the shunting-yard algorithm when we discuss the implementation in the
next section. 
...
\mytt{toInt}, that is used to convert a string into a natural number, provided the string can be interpreted as a
natural number.  For example, the string \mytt{'123'} is converted into the natural number $123$, while the
string \mytt{'**'} is returned unchanged.  Figure \ref{fig:toInt.py} on page \pageref{fig:toInt.py} shows the
implementation of this function:  The function \mytt{int} converts $s$ into a number, provided $s$ the string
$s$ represents an integer.  Otherwise, an Exception of type \mytt{ValueError} is raised.  This exception is
...
\end{minted}
\vspace*{-0.3cm}
\caption{The function \mytt{toInt}.}
\label{fig:toInt.py}
\end{figure}

The function \mytt{tokenize} receives a string $s$ representing an arithmetic expression and splits this
string into a list of numbers and operators. For example, the string \mytt{'1+2*3-4'} is transformed into the list
\\[0.2cm]
\hspace*{1.3cm}
...
\begin{enumerate}[(a)]
\item The ``\mytt{r}'' in front of the apostrophe ``\mytt{'}'' specifies that the regular expression is
...
\item The regular expression itself is divided into three parts.
      ...
      \begin{enumerate}
      \item ``\mytt{[0-9]+}'' matches a sequence of digits.  For example, it matches ``\mytt{0}'' or
            ``\mytt{123}''.  It would also match a string like ``\mytt{007}''.
            ...
            Inside of the expression, the ``\mytt{-}'' serves as a \blue{range operator}.
     ...
     \end{enumerate}
\end{enumerate}
The method call \mytt{re.findall(regExp, s)} scans the string $s$ for every substring that matches the
...
\end{minted}
\vspace*{-0.3cm}
\caption{The function \mytt{tokenize}.}
...
member variables.
\begin{enumerate}
\item \mytt{mTokens} is a stack of input tokens.  The operator symbols and parentheses are
      represented as strings, while the numbers are represented as rational numbers.  Hence, \mytt{mTokens}
      represents the \blue{token stack}.
\item \mytt{mArguments} is a stack of rational numbers.  Therefore, \mytt{mArguments} represents the
      \blue{argument stack}.
\item \mytt{mOperators} is the \blue{operator stack} containing arithmetic operators.  These operators
      are represented as strings.
\end{enumerate}
In order to implement the shunting-yard algorithm we need a number of auxiliary functions.
We start with the function \mytt{precedence} shown in Figure \ref{fig:precedence}.
Given an operator $o$, the expression $\mytt{precedence}(o)$ returns the \blue{precedence} of the operator
...
\end{minted}
\vspace*{-0.3cm}
\caption{The function \mytt{precedence}.}
...
The expression $\mytt{isLeftAssociative}(o)$ is \mytt{True} iff the operator $o$ 
\blue{associates to the left}, \index{associate to the left} i.e.~an expression of the form $x \;\mytt{o}\; y \;\mytt{o}\; z$
is evaluated as  $(x \;\mytt{o}\; y) \;\mytt{o}\; z$.  If $o$ \blue{associates to the right},
\index{associate to the right} then the expression  $x \;\mytt{o}\; y \;\mytt{o}\; z$
is evaluated as  $x \;\mytt{o}\; (y \;\mytt{o}\; z)$ and $\mytt{isLeftAssociative}(o)$
is \mytt{False}.  From the arithmetical operators that we consider, only the power operator ``\mytt{**}'' 
...
Figure \ref{fig:isLeftAssociative} shows the implementation of the function \mytt{isLeftAssociative}.
...
\end{minted}
\vspace*{-0.3cm}
\caption{The function \mytt{isLeftAssociative}.}
\label{fig:isLeftAssociative}
\end{figure}
The next auxiliary function we discuss is the function \mytt{evalBefore}.  This function is called with two
operators as arguments.  The expression $\mytt{evalBefore}(o_1, o_2)$ is \mytt{True}
...
consideration.   The precise behaviour of the function \mytt{evalBefore} is specified by the following rules:
\begin{enumerate}
...
\end{enumerate}
The implementation of \mytt{evalBefore} is shown in Figure \ref{fig:evalBefore}.
...
\end{minted}
\vspace*{-0.3cm}
\caption{The function \mytt{evalBefore}.}
\label{fig:evalBefore}
\end{figure}

Now we are ready to define the class \mytt{Calculator} that we will use to implement the shunting-yard
...
\begin{enumerate}
\item \mytt{mTokens} is the token stack.  The constructor initializes this stack by pushing all the tokens
      read by the tokenizer onto this stack.  As the list of tokens is reversed in the method
      \mytt{createStack}, the token that has been read first is on top of the stack.
\item \mytt{mOperators} is the operator stack.  Initially this stack is empty.
\item \mytt{mArguments} is the argument stack.  Initially this stack is empty.
\end{enumerate}
Furthermore, the class defines the method \mytt{evaluate} and the method \mytt{popAndEvaluate}.
...
\end{minted}
\vspace*{-0.3cm}
  \caption{The class \mytt{Calculator}.}
...
\blue{rewrite rules} that describe how the three stacks \mytt{mTokens}, \mytt{mArguments} and \mytt{mOperators}
...
\item If the token on top of the token stack is an integer, it is removed from the token stack and pushed onto
      ...
      $$\begin{array}{lcll}
       ...
       \end{array} 
     $$
      Here, the primed variable $\mytt{mArguments}'$ refers to the argument stack after \mytt{token} 
...
\end{enumerate}
The implementation of the method \mytt{evaluate} is shown in Figure \ref{fig:evaluate} on page \pageref{fig:evaluate}.
...
\end{minted}
\vspace*{-0.3cm}
  \caption{The method \mytt{evaluate}.}
  \label{fig:evaluate}
\end{figure} 
...
\end{minted}
\vspace*{-0.3cm}
\caption{The method \mytt{popAndEvaluate}.}
...
\item Extend the program discussed in these lecture notes so that it can also be used to evaluate
      ...
      \\[0.2cm]
      \hspace*{1.3cm}
      \mytt{sqrt}, \mytt{exp}, and \mytt{log}.
\item Extend the given program so that the arithmetic expressions may also contain 
      the strings ``\mytt{e}'' and ``\mytt{pi}'', where ``\mytt{e}'' stands for 
      \href{http://en.wikipedia.org/wiki/E_(mathematical_constant)}{Euler's number} 
      while ``\mytt{pi}'' stands for the mathematical constant
      \href{http://en.wikipedia.org/wiki/Pi}{$\pi$} defined as the ratio of the circumference of a
      circle to its diameter. 

      Note that these constants can be accessed in \textsl{Python} via the expressions \mytt{math.e}
      and \mytt{math.pi} provided the module \mytt{math} has been imported.
...
\end{enumerate}
In \textsl{Python} there is the predefined function \mytt{eval} which can be used to evaluate a string
containing an expression.  Furthermore, \textsl{Python} has the predefined function \mytt{exec}, that can be
...
We finish this chapter with a short discussion of the benefits of abstract data types.
 ...
 \item The use of abstract data types separates an algorithm from the data structures that
       ...
       how the data type \mytt{Stack} that we have used was implemented.  It was sufficient for us to know 
       \begin{enumerate}
 ...
 \item Abstract data types are \blue{reusable}.
       ...
       Modern industrial strength programming languages like \mytt{C++}, \textsl{Python}, or \textsl{Java} contain
...
\item Are you able to implement the \blue{shunting yard algorithm} in a language like \mytt{C} or \textsl{Java}.
\end{enumerate}
