...
\item $\textsl{Ts}$ is a set of \blue{type specifications}.  For every function symbol
      ...
      then $f$ is called a \blue{constructor}\index{constructor} of the data type $T$.  Otherwise,
      $f$ is called a  \blue{method}.\index{method}
...
and only the plate on top is accessible.  Formally, we define the data type \texttt{Stack}\index{Stack} as follows:
\begin{enumerate}
...
\item In \textsl{Python}, the constructor of a class has the name \texttt{\_\_init\_\_}.
      ...
      to initialize the \blue{member variables} of this object.  The class stack uses only one member variable:
...
\item We remove the operator \texttt{\symbol{34}*\symbol{34}} from the  token stack and compare the
      \blue{precedence} of the operator with the precedence of the operator \texttt{\symbol{34}+\symbol{34}},
...
known as \blue{operator precedence parsing}.
...
Next we fix the data structures
