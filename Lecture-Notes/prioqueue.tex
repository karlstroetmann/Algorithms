...
\item Before we are able to specify the behaviour of the functions implementing the function symbols
      ...
      \item The function $\texttt{insertList}$ takes a pair consisting of a priority and a value and inserts it
            ...
            \\[0.2cm]
            \hspace*{-0.8cm}
            $\texttt{insertList}: \texttt{Priority} \times \texttt{Value} \times \texttt{List}(\texttt{Priority} \times \texttt{Value}) 
            \rightarrow \texttt{List}(\texttt{Priority} \times \texttt{Value})$.
            \\[0.2cm]
      ...
      \begin{enumerate}
      \item $\texttt{prioQueue}().\texttt{isEmpty}() = \texttt{true}$

            The constructor $\texttt{prioQueue}$ creates an empty queue.
      \item $Q.\texttt{insert}(p, v).\texttt{isEmpty}() = \texttt{false}$

            If something is inserted into a priority queue, the resulting queue is not empty.
      \item $\texttt{prioQueue}().\texttt{top}() = \Omega$
      ...
      \item $\neg Q.\texttt{isEmpty}() \rightarrow Q.\texttt{top}() = Q.\texttt{toList}()[1]$

            If we retrieve the  pair with the highest priority from an non-empty priority queue $Q$,
            we get the same pair that is the first element of the list $Q.\texttt{toList}()$.

      \item $Q.\texttt{isEmpty}() \rightarrow Q.\texttt{remove}() = Q$

            Trying to remove the top element from an empty queue $Q$ returns $Q$.
      \item $\neg Q.\texttt{isEmpty}() \rightarrow Q.\texttt{remove}().\texttt{toList}() =
        Q.\texttt{toList}()[2..]$
...
\item $Q.\texttt{insert}(p,v)$ is implemented by the function $\texttt{insertList}$.  Note that we have
      implemented this function already when we had implemented the sorting algorithm ``\blue{insertion sort}''.
\item $Q.\texttt{top}()$ return the first element from the list $Q$.
...
Given an array $\texttt{A}$ of keys to be sorted, the basic version of heapsort proceeds as follows:
\begin{enumerate}
