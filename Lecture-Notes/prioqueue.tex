...
\blue{priority queue} is more like a dentist's waiting room: You have an appointment at 10:00 and you
...
\begin{enumerate}[(a)]
\item We formally define the abstract data type \mytt{PrioQueue} that describes priority queues.
\item We present the data structure of a \blue{heap}.  This data structure is one way to implement
      priority queues.
\item Next, we implement \blue{heaps} in \textsl{Python}.
\item Finally, we present \blue{HeapSort}, which is a sorting algorithm that is based on heaps.
\end{enumerate}
...
\item $l = \mytt{Node}(p_1,v_1,l_1,r_1) \;\wedge\; r = \mytt{Node}(p_2,v_2,l_2,r_2) \;\wedge\; p_1 > p_2 \rightarrow$ \\[0.1cm]
      ...
      the root.
\end{enumerate}
The hawk-eyed reader will have noticed that the specification of the method $\mytt{remove}$ that is given
...
memory. While \blue{merge sort} needs only $n \cdot \log_2(n)$ comparisons to get the job done, it
...
\item The procedure $\mytt{sink}$ takes three arguments.
      ...
      violated at position ${j}$.  Therefore,  ${k}$ is set to ${j}$ and the \mytt{while}-loop repeats
...
\item How can we implement the methods $\mytt{insert}$ and \mytt{remove} if we implement a priority queue as a
      heap? 
