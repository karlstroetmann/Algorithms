...
\item Before we are able to specify the behaviour of the functions implementing the function symbols
      ...
      \item The functions \mytt{top} and \mytt{isEmpty} are \blue{observers} of the \textsc{Adt}
            \mytt{PrioQueue}:  They do not \blue{change} a priority queue, instead they return information
            about a priority queue.  These observer function can be reduced to the more general observer
            function \mytt{toList}. 
...
\end{enumerate}
\end{Definition}
We could implement the ADT $\mytt{PrioQueue}$ as a list of pairs that is sorted ascendingly.
