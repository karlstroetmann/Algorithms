...
\begin{Definition}[Priority Queue] \index{priority queue} \hspace*{\fill} \\
  ...
  \item The signatures of these function symbols is given as follows:
        ...
        \item $\texttt{remove}: \texttt{PrioQueue} \rightarrow \texttt{PrioQueue}$
              ...
              The expression $Q.\texttt{remove}()$ removes from $Q$ that element that is returned by
              $Q.\texttt{top}()$.
...
\item Before we are able to specify the behaviour of the functions implementing the function symbols
      ...
      \item The function \texttt{remove} is a \blue{mutator} function\index{mutator function}: It does
...
\item $\texttt{Node}(p,v,l,r) \in \mathcal{H}$ if and only if the following is true:
      \begin{enumerate}
      \item $p \in \mathtt{Priority} \wedge v \in \mathtt{Value}$.
      ...
      \item $\mid l.\texttt{count}() - r.\texttt{count}() \mid \;\leq\, 1$
            ...
            $l.\mathtt{count}()$ computes the number of nodes of the heap $l$ and $r.\mathtt{count}$ is
            interpreted similarly.  A formal definition of the function \texttt{count} is given below.
            The condition 
            \\[0.2cm]
            \hspace*{1.3cm}
...
\\[0.2cm]
\hspace*{1.3cm}
$\texttt{count}:\mathcal{H} \rightarrow \mathbb{N}$
\\[0.2cm]
...
The class \texttt{Heap} maintains the static variable \texttt{sNodeCount}.  This variable is used to attach a
...
\item The procedure $\texttt{sink}$ takes three arguments.
      ...
      \item ${n}$ is the upper bound  of the part of this array that has to be transformed into a heap.  
            ...
            ${A[k:n+1]}$. 
      \end{enumerate}
      When calling $\texttt{sink}$, the assumption is that $A[{k:n+1}]$ should represent a heap 
      ...
      $2 \cdot {k} + 1 \leq {n}$.
\item The function $\texttt{heapSort}$ has the task to sort the array ${A}$ and proceeds in two phases.
      \begin{enumerate}
      \item In phase one our goal is to transform the array ${A}$ into a heap that is stored in ${A}$.
            ...
            \texttt{for}-loop starting in line 16.  The invariant of this loop is that before
            ...
            In order to maintain the invariant for index ${k}$, $\texttt{sink}$ is called with
            argument ${k}$,  since at this point, the tree rooted at index ${k}$ satisfies
            the heap condition except possibly at its root.  It is then the job of $\texttt{sink}$ to
      ...
      \item In phase two we remove the elements from the heap one-by-one and insert them at the end of
            ...
            returned in the end.  In order to reestablish the heap condition at the root, we call $\texttt{sink}$ with index
            \texttt{0} in line 21.
...
\item Assume that we have a method $\texttt{toList}$ that transforms a priority queue into
...
\item How can we define the methods $\texttt{insert}$ and \texttt{remove} if we implement a priority queue as a
      heap? 
