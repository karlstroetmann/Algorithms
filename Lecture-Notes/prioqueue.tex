...
\end{enumerate}
Next, we define the set $\mathcal{H}$ of heaps by induction:
\begin{enumerate}
...
\item The procedure $\texttt{heapSort}$ has the task to sort the array ${A}$ and proceeds in two phases.
      ...
      \item In phase one our goal is to transform the array ${A}$ into a heap that is stored in ${A}$.
            ...
            \item Case: $n$ is even, i.e.~there is an $m\in \mathbb{N}$ such that $n = 2 \cdot m$.
                  ...
                  \\[0.2cm]
                  As $2 \cdot k  + 1$ is greater than $n$ is the last index in an array of length $n+1$, the
            ...
            \item Case: $n$ is odd, i.e.~there is an $m \in \mathbb{N}$ such that $n = 2 \cdot m + 1$.
                  ...
                  \\[0.2cm]
                  \hspace*{1.3cm}
                  $2 \cdot k + 1 = 2 \cdot (m + 1) = 2 \cdot m + 2 = n + 1 > n$.
                  \\[0.2cm]
      ...
      \item In phase two we remove the elements from the heap one-by-one and insert them at the end of
            ...
            last element of the array ${A}$ is already in its correct position in the list taht is to be
