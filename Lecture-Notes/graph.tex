...
\begin{Definition}[Kürzeste-Wege-Problem] \lb
...
\begin{enumerate}
\item[I.A.:]  
      Zu Beginn enthält die Menge $\textsl{visited}$ nur den Knoten \textsl{source} und offenbar gilt
      \\[0.2cm]
      \hspace*{1.3cm}
      $\textsl{sp}(\textsl{source}) = 0 = \textsl{dist}(\textsl{source})$.
      \\[0.2cm]
      Daher stimmt die Behauptung für alle Elemente, die zu Beginn Elemente der Menge \textsl{visited} sind.
\item[I.S.:]
      ...
      $\textsl{dist}[u] \geq \textsl{sp}(u)$ gilt, denn kein Pfad kann kürzer als der kürzeste Pfad sein, gilt insgesamt
      \\[0.2cm]
      \hspace*{1.3cm}
...
\\[0.2cm]
analysieren.
Die Anzahl dieser Aufrufe ist durch die Anzahl der Kanten begrenzt, die zu dem Knoten $v$ hinführen.
