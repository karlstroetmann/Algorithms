\section{AVL Trees}
If a binary tree is approximately \blue{balanced}, that is~if the left and right subtree of a binary tree $b$  have
...
In order to define these trees we need to define the \blue{height} of an ordered binary tree $b$ formally.  To this
...
{\em 
  The set $\AVL$ of \blue{\textsc{Avl} trees}\index{AVL tree} \index{$\mathcal{A}$, \textsc{Avl} tree} is defined inductively:
  \begin{enumerate}
...
\item $\begin{array}[t]{cl}
      ...
      easily verified:
      \begin{enumerate}[(a)]
      \item First, consider the node containing the key $k_2$.
            The left subtree $l_2$ of this node has the height $h$ and the right subtree
            that has the key $k_1$ at its root either has the height $h$ or $h+1$.
            This height is $h+1$ if $r_2$ has the height $h$.  It is $h$ if the height of $r_2$ is $h-1$.
      \item Second, the node containing the key $k_1$ is balanced because the heights of $r_2$ and $r_1$ differ
            at most by one.
      \end{enumerate}
...
member variable \mytt{mHeight} to our class \mytt{AvlTree}.
...
Figure \ref{fig:avl-tree.ipython:delete} shows the implementation of the method $\mytt{delete}$.
...
\item Specify a method $t.\mytt{insert}(k)$ that inserts the key $k$ into the 2-3 tree
      ...
      \item $t.\mytt{ins}(k)$ takes a 2-3 tree $t$ and a key $k$ and inserts the key $k$ into $t$.
            ...
            The function \mytt{ins} is recursive and uses the function \mytt{restore} that is described next.
...
In order to implement string completion we first define the function \mytt{allKeys}, which
...
\exercise
(\textbf{Binary Tries})  Let us assume that our alphabet is the \blue{binary alphabet}, that is the alphabet
...
\item Similarly, the method \mytt{delete} is a wrapper of the method \mytt{delete} from the class
      \mytt{ListNode}.  If the given list is not empty, that is \mytt{mPtr} is not \mytt{None},
