...
Form von Such-Bäumen, die an diese Situation besonders angepasst ist.  Diese Such-Bäume
...
nur zwei Nachfolger hat, wie das bei binären Bäumen der Fall ist, sondern stattdessen
...
\item $\texttt{Node}(v, [c_1, \cdots, c_n], [t_1, \cdots, t_n]).\textsl{find}(cr) = 
      ...
      übereinstimmt und wenn außerdem der Trie $t_i$ einen Wert zu dem
      Schlüssel $r$ enthält.
\end{enumerate}
...
Außerdem markieren wir die diese Knoten darstellenden Kreise in den oberen Hälften 
...
\\[0.2cm]
für ein Wort $w\in \Sigma^*$ und einen Wert $v \in \textsl{Value}$ durch Induktion nach der Länge des Wortes $w$.
\begin{enumerate}
...
$t.\textsl{delete}(w)$
\\[0.2cm]
für einen Trie $t \in \mathbb{T}$ und ein Wort $w \in \Sigma^*$
