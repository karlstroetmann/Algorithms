...
trees we need to define the \blue{height} of a binary tree formally:
\begin{enumerate}
...
\raisebox{-4pt}{\epsfig{file=Abbildungen/google.eps, scale=0.1}}.
...
\hspace*{1.3cm} 
$\mathtt{code}: \Sigma^* \rightarrow \mathbb{N}$ \\[0.2cm]
as follows: \\[0.2cm]
\hspace*{1.3cm} 
...
\item We have to change the function  $\mathtt{code}$ so that the result of this function is always
      ...
      $\mathtt{code}(c_0c_1\cdots c_n) = \left(\sum\limits_{i=0}^n \mathtt{ord}(c_i) \cdot
        27^i\right) \;\%\; \mathtt{size} + 1$,
      \\[0.2cm]
      then we will always have $\mathtt{code}(c_0c_1\cdots c_n) \leq \mathtt{size}$.  In order to
      prevent overflows we can define the partial sum $s_k$ for  $k=n,n-1,\cdots,1,0$ by induction:
      \begin{enumerate}
      \item $s_n = \mathtt{ord}(c_n)$,
      \item $s_{k} = \bigl(\mathtt{ord}(c_{k}) + s_{k+1} \cdot 27 \bigr) \;\%\; \mathtt{size}$.
      \end{enumerate}
      Then we have
      \hspace*{1.3cm} 
      $s_0 + 1 = \left(\sum\limits_{i=0}^n \mathtt{ord}(c_i) \cdot 27^i\right) \;\%\; \mathtt{size} + 1$.
...
Figure \ref{fig:hashTable.stlx-outline} shows the outline of the class $\mathtt{hashTable}$.  
\begin{enumerate}
...
\item Our implementation maintains two static variables.
  ...
  \item $\mathtt{sPrimes}$ is a list of prime numbers.  Roughly, these prime numbers double in size.
        The reason is that performance of a hash table is best if the size of $\mathtt{mArray}$ is a
...
quite a bit faster than \textsl{AVL}-trees or \blue{red-black} trees.  However, this is only true if
