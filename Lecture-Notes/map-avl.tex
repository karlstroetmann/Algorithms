...
string that has more than five characters.  In \textsl{Python}, integers are unbounded.  However, a computation
using big numbers is considerably slower than a computation that uses numbers that fit into a single memory word.
Hence we should try to reorganize the computation of that happens in equation (\ref{eq:hash1}).  The should be
possible as the final result of $\texttt{hash\_code}(c_0c_1\cdots c_{k-1}, n)$ is bounded by $n$.    In order
to be able to reorganize the computation we need to discuss
...
\proof
If we define $q_1 := (a + b) \mdiv n$, the according to the division theorem we have 
\\[0.2cm]
\hspace*{1.3cm}
...
Figure \ref{fig:hash-example} on page \pageref{fig:hash-example} an outline of the implementation of a hash map.  This data
structure is called a \href{https://en.wikipedia.org/wiki/Hash_table}{hash table}. 
...
Figure \ref{fig:hashTable.ipython-outline} on page \pageref{fig:hashTable.ipython-outline} shows the outline of the
...
\item $\texttt{mAlpha}$ is the \blue{load factor} of our hash table.  If at any point in time, we have
      \\[0.2cm]
      \hspace*{1.3cm}
...
Figure \ref{fig:hashTable.ipython-hashCode} gives the implementation of the method $\texttt{hashCode}$.
\begin{enumerate}
...
Figure \ref{fig:hashTable.ipython-find} shows the implementation of the method $\texttt{find}$.
\begin{enumerate}
