...
\hspace*{1.3cm} $\mytt{Key} = \{ 0, 1, 2, \cdots, n \}$. \\[0.2cm]
...
\item The method \mytt{delete} removes the value stored at the index $k$ by overwriting it with the undefined
      value. 
\end{enumerate}
If the domain $D := \mytt{dom}(f)$ of the function $f$ is not a set of the form $\{0,1, \cdots, n\}$, 
...
\begin{Theorem}[Euclidean Division]
  ...
  \item $0 \leq r < n$.
  \end{enumerate}
  The number $q$ is called the \blue{quotient} of $a$ divided $n$ and denoted as $a \;\mytt{//}\; n$, while
...
\begin{enumerate}[(a)]
\item \mytt{mKey}     stores the \emph{key},
\item \mytt{mValue}   stores the \emph{value} associated with this key, and
...
\item Furthermore, the class implements the function \mytt{\_\_iter\_\_} which returns a
      ...
      use a \mytt{for}-loop to iterate over the list that is stored.  
\end{enumerate}
...
\item Finally, we look up the information stored under the given $\mytt{key}$ in this key-value list.
      Since the class \mytt{ListMap} is iterable we can use a \mytt{for}-loop to loop over the key-value
      pairs and return the value associated with the given \mytt{key}.
\end{enumerate}
