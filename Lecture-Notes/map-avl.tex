...
\item Specify a method $t.\texttt{insert}(k)$ that inserts the key $k$ into the 2-3 tree
      $t$.  You should make use of an auxiliary function $t.\texttt{restore}()$ that takes a 2-3-4
...
store any key.  The following formul\ae\ specify the function $\texttt{isEmpty}$:
\begin{enumerate}
\item $\texttt{Node}(\Omega, [],[]).\texttt{isEmpty}() = \texttt{true}$,
\item $v \not= \Omega \rightarrow 
       \texttt{Node}(v, [c_1,\cdots,c_n],[t_1,\cdots,t_n]).\texttt{isEmpty}() = \texttt{false}$,
\item $\texttt{Node}(\Omega, L, T).\texttt{isEmpty}() = \texttt{isEmptyList}(T)$
\end{enumerate}
In the last formula we have used the auxiliary function
\\[0.2cm]
\hspace*{1.3cm}
$\texttt{isEmptyList}: \texttt{List}(\mathbb{T}) \rightarrow \mathbb{B}$.
\\[0.2cm]
For a list of tries, this function checks whether all tries in this list are empty.  Formally,
$\texttt{isEmptyList}(l)$ is defined by induction on the length of the list $l$.
\begin{enumerate}
\item $\texttt{isEmptyList}\bigl([]\bigr) = \texttt{true}$,
\item $\texttt{isEmptyList}\bigl([t] + r\bigr) = \bigl(t.\texttt{isEmpty}() \wedge \texttt{isEmptyList}(r)\bigr)$,
...
If the domain $D := \texttt{dom}(f)$ of the function $f$ is not a set of the form $\{1, \cdots, n\}$, 
