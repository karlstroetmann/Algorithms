...
\subsection{Implementing \textsc{Avl}-Trees in \textsl{Python}}
...
Figure \ref{fig:avl-tree.ipython:init} shows the constructor of the class \texttt{\textsc{Avl}Tree}.  The variable
...
A tree is empty if its height is $0$.
...
\ref{fig:binary-tree.py-1}.  The reason is that every \textsc{Avl} tree is also an ordered binary tree and
...
\begin{minted}[ frame         = lines, 
    ...
    def insert(self, key, value):
        if self.isEmpty():
            self.mKey    = key
            self.mValue  = value
            self.mLeft   = \textsc{Avl}Tree()
            self.mRight  = \textsc{Avl}Tree()
            self.mHeight = 1
        elif self.mKey == key:
            self.mValue = value
        elif key < self.mKey:
...
\subsection{Analysis of the Complexity of \textsc{Avl} Trees}
...
it can possibly be while still satisfying the definition of an \textsc{Avl} tree.  Hence the worst case
...
\item All keys stored in  $b_h(k)$ are natural number that are bigger than  $k$.
      ...
      not really matter, we define them to be $0$.
\item If $h \geq 1$, then the height of the left subtree of $b_h(k)$ is bigger than the height of its right subtree.
\end{enumerate}
...
\\[0.2cm]
Given an ordered binary tree  $b$, the expression $b.\texttt{maxKey}()$ returns the biggest
...
\item $b_{h+1}(k).\texttt{maxKey}() = l \rightarrow 
      ...
      possible we first construct the \textsc{Avl} tree $b_{h+1}(k)$ which has height  $h+1$ and which stores as few
      ...
      must at least have height $h$ in order for the whole tree to be an \textsc{Avl} tree.
\end{enumerate}
...
Hence, for \textsc{Avl} trees the complexity of $b.\texttt{find}(k)$ is logarithmic even in the worst case.
Figure 
