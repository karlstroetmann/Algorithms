...
{\em 
  The set $\AVL$ of \blue{AVL trees}\index{AVL tree} is defined inductively:
  \begin{enumerate}
...
\subsection{Additional Methods Supported by Tries}
...
\\[0.2cm]
Here, $p+w$ denotes the concatenation of the strings $p$ and $w$ and the expression $w \in T$ is true iff  $T.\texttt{find}(w) \not= \Omega$.
...
argument, then it returns the set of all keys that are stored in the given trie.  The argument $p$ is needed
...
and, furthermore, have the prefix $s$.  Additionally, it replaces the prefix $s$ with $p$.  Therefore
we have
\\[0.2cm]
\hspace*{1.3cm}
$T.\texttt{\_findSuffix}(s, p) = \bigl\{ p+r \mid s + r \in T \bigr\}$.
\\[0.2cm]
The function \texttt{\_findSuffix} can be implemented by induction with respect to its second argument:
\begin{enumerate}[(a)]
