\chapter{Recurrence Equations and Sums}
...
\begin{Definition}[Second Order Linear Inhomogeneous Recurrence Equation] \hspace*{\fill} \\
  ...
  \\[0.2cm]
  Here, $c$ is the \blue{inhomogeneity} and $x_0 = d$, $x_1 = e$ are the \blue{initial conditions}.\index{initial conditions}  Taken together, the recurrence
...
\item In order to compute a \blue{special solution} \index{special solution} of the \blue{inhomogeneous} recurrence equation
      ...
      \item $a + b \not= 1$.
            ...
            $
            \\[0.2cm]
            This equation has a unique solution if and only if the
            \href{https://en.wikipedia.org/wiki/Determinant}{determinant}\footnote{
              The notion of a determinant is discussed in the lecture on \textsl{Linear Algebra}.}
              of the matrix is different from 
            $0$.  We have
            \\[0.2cm]
            \hspace*{1.3cm}
            $
...
\section{Finite Sums of Integer Powers}
...
\\[0.2cm]
\hspace*{1.3cm}
$ \bigl(i^3 - (i-1)^3\bigr) = i^3 - \bigl(i^3 - 3 \cdot i^2 + 3 \cdot i - 1) = 3 \cdot i^2 - 3 \cdot i + 1$.
\\[0.2cm]
Hence we have
\\[0.2cm]
\hspace*{1.3cm}
