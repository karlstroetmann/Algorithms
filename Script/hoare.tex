...
\item Wir betrachten wieder as Programm-Fragment  ``\texttt{x = x + 1;}'' und wählen als
      ...
      \[ \hoare{\textsl{prim}(x)}{x := x + 1;}{\textsl{prim}(x+1)} \] 
      haben müsste.  Das diese letzte Zeile falsch ist, können wir sehen, wenn wir für $x$
...
positiv sind.  Würde die Schleife unendlich lange laufen, so müsste $s$ irgendwann negative
...
aufwendig.  Von Hand können nur Programme verifiziert werden, die in der selben
...
Dabei müssen wir allerdings berücksichtigen, dass ein und das selbe textuelle
