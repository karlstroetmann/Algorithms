...
\item $\begin{array}[t]{cl}
       ...
       $
        ...
        $h-1$, wobei mindestens einer der beiden Teilbäume die Höhe $h$ haben muß.
\begin{figure}[!ht]
  \centering
...
Balancierungs-Bedingung wiederherstellen läßt, wenn diese durch eine Einfüge- oder
...
\item Am Ende der Methode \textsl{insert}() wird in Zeile 43 die Methode
      ...
      Eigentlich müßte die Methode $\textsl{restore}()$ nur dann aufgerufen werden,
...
\item In Zeile 84 implementieren wir die Methode $\textsl{restore}()$.
      \begin{enumerate}
      \item Falls die Balancierungs-Bedingung bereits erfüllt ist, so muß die Methode
...
maximal wird.  Das ist aber das selbe wie wenn in einem Baum gegebener Höhe die Zahl der
...
\item $C = [c_1, \cdots, c_n] \in \textsl{List}(\Sigma)$ eine Liste von Buchstaben der
      Länge $n$ ist,
\item $T = [t_1, \cdots, t_n] \in \textsl{List}(\mathbb{T})$ eine Liste von Tries der selben Länge $n$ ist, 
\end{enumerate}
dann gilt \\[0.2cm]
...
Die Funktion, die hier dargestellt wird, läßt sich wie folgt als binäre Relation
schreiben: \\[0.2cm]
...
\item $\textsl{node}\bigl(v_1,[c_1,\cdots,c_i,\cdots,c_n], [t_1,\cdots,t_i,\cdots,t_n]\bigr).\textsl{insert}(c_ir,v_2) =$ \\[0.2cm]
      ...
      an der Stelle $i$ vorkommt, wenn also gilt $c= c_i$, dann muß der Wert $v_2$
...
in diesem Fall trivial implementieren läßt.
...
\item Als nächstes übersetzen wir Namen in einen Index.  
      ...
      \\[0.2cm]
      so läßt sich der Wert eines Strings $w = c_0c_1\cdots c_7$ durch eine Funktion \\[0.2cm]
      \hspace*{1.3cm} 
...
\item Als erstes überlegen wir uns, welche Daten-Strukturen wir brauchen,
      ...
      \item Wenn die einzelnen Listen zu groß werden, wird die Suche ineffizient.
            ...
            Um diesen Prozeß zu steuern, müssen wir zunächst nachhalten, 
...
\item Als nächstes besprechen wir das \emph{Rehashing}, das in Zeile 55 -- 69
      ...
      Zum Schluß kopieren wir in Zeile 67  das Feld der neu angelegten Hash-Tabelle in die
...
\item \texttt{boolean remove(Object $e$)}
      ...
      sonst \texttt{false}.
      Falls $c$ eine Menge ist, so läßt sich die Semantik durch folgende Gleichungen
      spezifizieren.
      \begin{enumerate}
...
\item \texttt{boolean retainAll(Collection<?> $d$)}
      ...
      $c$ entfernt wurden.  Falls $c$ und $d$ Mengen sind, läßt sich die Semantik wie
      folgt spezifizieren:
      \begin{enumerate}
...
\item Die Schnittstelle \texttt{Set<E>} beschreibt Mengen.  Mengen sind Zusammenfassungen, die
      ...
      der Menge miteinander zu vergleichen, so läßt sich eine Menge in \textsl{Java}
...
\item \texttt{TreeMap(Map<K,V> $m$)}
      ...
      Dieser Konstruktor erzeugt eine neue Abbildung, die die selben Zuordnungen enthält 
