...
\item Algorithmus: 
  \begin{enumerate}
  \item Konzept für systematisches Lösen eines Problems
  \item abstrakt
  \item oft: Darstellung als Pseudo-Code
  \item auch: natürlich-sprachlicher Text
...
\item $t$ lässt sich vom C-Compiler fehlerfrei parsen
\end{enumerate}
...
\tiny \addtocounter{mypage}{1}
\rule{17cm}{1mm}
Komplexität  \hspace*{\fill} Seite \arabic{mypage}
\end{slide}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Ausführen: \quad$\;$ \texttt{java Fibonacci}
...
\item Fall: $\mathtt{sp}(\chi) \not= 0$.
      ...
      Ansatz für spezielle Lösung: \quad $a_n = \delta$
...
\item Fall: $\mathtt{sp}(\chi) = 0$, $\chi'(1) \not= 0$.
      ...
      Ansatz für spezielle Lösung: \quad $a_n = \varepsilon \cdot n$
...
\item Fall: $\mathtt{sp}(\chi) = 0$, $\chi'(1) = 0$,  $\chi''(1) \not= 0$.
      ...
      Ansatz für spezielle Lösung: \quad $a_n = \varepsilon \cdot n^2$
...
\footnotesize
allg.~Lösung inhomogenene lineare Rek.~Gl.: \\[0.3cm]
\hspace*{1.3cm}
...
\item Berechne zusätzliche Anfangs-Bedingungen: 
...
\item Rekurrenz-Gleichungen für Anzahl 
      \\[0.3cm]
      \hspace*{1.3cm}
...
\item $g:\N \rightarrow \R_+$ genügt der Rekurrenz-Gleichung 
      \\[0.2cm]
      \hspace*{1.3cm}
...
\tiny \addtocounter{mypage}{1}
\rule{17cm}{1mm}
Sortieren durch Einfügen  \hspace*{\fill} Seite \arabic{mypage}
\end{slide}
...
\begin{center}
Eigenschaften des Algorithmus \\ \textsl{Sortieren durch Einfügen}
\end{center}
\vspace*{0.5cm}

\footnotesize
\begin{enumerate}
\item Distributivität von \texttt{le} über \texttt{insert}

      $\mathtt{le}\bigl(x, \mathtt{insert}(y,L) \bigr) \leftrightarrow x \preceq y \wedge \mathtt{le}(x,L)$. 
\item Transitivität von \texttt{le}
...
\item Distributivität von \texttt{isSorted} über \texttt{insert}
...
\item Distributivität von \texttt{count} über \texttt{insert}

      $\mathtt{count}\bigl(x, \mathtt{insert}(y, S) \bigr) = \mathtt{eq}(x,y) + \mathtt{count}(x,S)$
\item Distributivität von \texttt{count} über \texttt{sort} 
...
\tiny \addtocounter{mypage}{1}
\rule{17cm}{1mm}
Sortieren durch Einfügen --- Korrektheit \hspace*{\fill} Seite \arabic{mypage}
\end{slide}
...
\tiny \addtocounter{mypage}{1}
\rule{17cm}{1mm}
Hoare-Kalkül \hspace*{\fill} Seite \arabic{mypage}
\end{slide}
...
\tiny \addtocounter{mypage}{1}
\rule{17cm}{1mm}
Symbolische Programm-Ausführung \hspace*{\fill} Seite \arabic{mypage}
\end{slide}
...
\tiny \addtocounter{mypage}{1}
\rule{17cm}{1mm}
Binäre Bäume \hspace*{\fill} Seite \arabic{mypage}
\end{slide}
...
\tiny \addtocounter{mypage}{1}
\rule{17cm}{1mm}
AVL-Bäume \hspace*{\fill} Seite \arabic{mypage}
\end{slide}
...
\footnotesize
\begin{enumerate}
\item Annahme: Namen haben Länge 8
      \begin{itemize}
      \item kürzere Namen mit Blanks auffüllen
      \item längere Namen abschneiden
      \end{itemize}
...
\item \texttt{Iterator<E> iterator()}
      ...
      $c.\mathtt{iterator}()$ liefert Iterator für Zusammenfassungen $c$

      ermöglicht erweiterte \texttt{for}-Schleife
      \\[0.2cm]
...
\item \texttt{E last()}
      ...
      $s.\mathtt{last}()$: \quad größtes Element von $s$.
\end{enumerate}
Konstruktoren
\begin{enumerate}
...
\footnotesize
\begin{enumerate}
\item Näherungsweise balancierte binäre Bäume.
...
\item Höhe:  rote Knoten werden nicht gezählt.
\item Linker und rechter Teil-Baum: selbe Höhe.
\end{enumerate}
...
\item \texttt{LinkedList}: \quad verkettete Listen
      ...
      Komplexität $l.\mathtt{get}(i)$, $l.\mathtt{set}(i,e)$: \quad linear

      Komplexität $l.\mathtt{add}(e)$: \hspace*{3.5cm} konstant
\item \texttt{ArrayList}: \quad Feld

      Komplexität $l.\mathtt{get}(i)$, $l.\mathtt{set}(i,e)$: \quad konstant

      Komplexität $l.\mathtt{add}(0,e)$: \hspace*{2.9cm} linear
\end{enumerate}
...
\item \texttt{boolean offer(E $e$)}
      ...
      $q.\mathtt{offer}(e)$: \quad einfügen von e $e$ am Ende
...
\setcounter{enumi}{7}
 \item \texttt{Set<K> keySet()}
      ...
      \emph{Ansicht} der Schlüssel (engl. \emph{view})
      \begin{enumerate}
      \item Löschen in $m.\mathtt{keySet}()$
...
\begin{enumerate}
\item natürlicher Vergleich: \quad \texttt{K implements Comparable<K>}
...
\tiny \addtocounter{mypage}{1}
\rule{17cm}{1mm}
Prioritäts-Warteschlangen \hspace*{\fill} Seite \arabic{mypage}
\end{slide}
...
\begin{center}
Prioritäts-Warteschlangen --- Axiome \\[0.4cm]
...
Komplexität von $Q.\textsl{insert}(k,v)$: \quad $\Oh(\#Q)$

\vspace*{\fill}
\tiny \addtocounter{mypage}{1}
\rule{17cm}{1mm}
Prioritäts-Warteschlangen: Verhalten \hspace*{\fill} Seite \arabic{mypage}
\end{slide}
...
\tiny \addtocounter{mypage}{1}
\rule{17cm}{1mm}
vollständige binäre Bäume \hspace*{\fill} Seite \arabic{mypage}
\end{slide}
...
\tiny \addtocounter{mypage}{1}
\rule{17cm}{1mm}
nahezu vollständige binäre Bäume \hspace*{\fill} Seite \arabic{mypage}
\end{slide}
...
\item $\weight{\cdot}: \edges \rightarrow \N \backslash\{0\}$ 
      ...
      Funktion, die jeder Kante positive \textsl{Länge} zuordnet.
\end{enumerate}
...
\tiny \addtocounter{mypage}{1}
\rule{17cm}{1mm}
Kürzeste Wege \hspace*{\fill} Seite \arabic{mypage}
\end{slide}
...
\footnotesize
\begin{enumerate}
\item Idee:  Problem durch zufällige Simulation lösen
\item Anwendungen
      \begin{enumerate}
      \item Berechnung komplexer Volumina oder allgemein mehrfacher Integrale
      \item Verhalten eines Systems hängt von zufälligen Einflüssen ab
...
\item Berechnung zufälliger Permutationen 
...
Merkmale:
\begin{enumerate}
\item gut geeignet für grobe Abschätzungen
\item aufwendig für hohe Genauigkeit
\end{enumerate}
