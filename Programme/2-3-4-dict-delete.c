...
// An estimate of the number of valid keys, i.e. keys that are alive. 
// This number is not exact, because we do not adjust it in the case
// that a key is overwritten.

static unsigned numberValid = 0;

// An estimate of the number of zombie keys, i.e. keys that have been deleted.
// This number is not exact because we do not adjust it if either a
// zombie key is reanimated or else a key is deleted that is not in the tree.
...
// Add a given key value pair to the node p.  It is guaranteed that p is not
...
// The functions insertAux() and splitAndInsert() are mutually recursive. 
// Therefore we need the forward declaration below.

Table insertAux(Table t, Key key, Value val);

// Insert the pair <k,v> into the tree t. The node p is the parent of t.
// There are two cases:
//   0. If t is empty, p must be a leaf.  Then we can add the key to this leaf
//      since it is guaranteed that p is not a 4-node. 
...
//      parent. 
//   2. Otherwise, i.e. if t is either a 2-node or a 3-node, <k,v> is 
//      inserted recursively in the tree t.
...
Table insertAux(Table t, Key key, Value val)
{
    ...
    case FOUR: {
		// This is the only case where the tree t can grow.
        NodePtr n = split4Node(t);
...
// Insert the pair <key, val> into the table t.  Return the resulting table.
...
// Delete a given key in the ordered 2-3-4 tree t. 
Table deleteAux(Table t, Key key) 
{
    if (t == 0) {
        return 0;
    }
...
// into keyArray and valArray. idx is used as an index into these arrays.
...
// Count the number of keys in the tree t that are alive.

unsigned countKeys(Tree t) 
{
    if (t == 0)
        return 0;
    switch (t->type) {
    case TWO: {
        unsigned count1 = countKeys(t->ptr1);
        unsigned count2 = countKeys(t->ptr2);
        unsigned key1   = (t->alive1 ? 1 : 0);
        return count1 + count2 + key1;
    }
    case THREE: {
        unsigned count1 = countKeys(t->ptr1);
        unsigned count2 = countKeys(t->ptr2);
        unsigned count3 = countKeys(t->ptr3);
        unsigned key1   = (t->alive1 ? 1 : 0);
        unsigned key2   = (t->alive2 ? 1 : 0);
        return count1 + count2 + count3 + key1 + key2;
    }
    case FOUR: {
        unsigned count1 = countKeys(t->ptr1);
        unsigned count2 = countKeys(t->ptr2);
        unsigned count3 = countKeys(t->ptr3);
        unsigned count4 = countKeys(t->ptr4);
        unsigned key1   = (t->alive1 ? 1 : 0);
        unsigned key2   = (t->alive2 ? 1 : 0);
        unsigned key3   = (t->alive3 ? 1 : 0);
        return count1 + count2 + count3 + count4 + key1 + key2 + key3;
    }
    }
    assert(0);
    return 0;
...
{
    unsigned length = countKeys(t);
    Key      keyArray[length];
    Value    valArray[length];
    unsigned idx = 0;
