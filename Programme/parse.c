...
void printFsm(FSM* fsm)
{
	...
	for (unsigned i = 0; i < n; ++i) {
		...
		} else if (fsm->symbol[i] == '\\') {
			fprintf(handle, "  %u -> %u [label = \"\\\\\"];\n", 
					i, fsm->next1[i]);
		} else {
...
{
	FSM* f1 = parseFactor();
	if (*charPtr != 0 && *charPtr != '+' && *charPtr != ')' && *charPtr != '*') {
		FSM* f2 = parseTerm();
		return concat(f1, f2);
	}
	return f1;
...
int main()
{
	// The length of the regular expression must not exceed 1000 characters.
	charPtr = malloc( 1001 * sizeof(char) );
	printf("RegExp: ");
	scanf("%s", charPtr);
	FSM* fsm = parseExpr();
	printFsm(fsm);	
	// The strings to test must not exceed 1000 characters.
	char buffer[1001];
	do {
		printf("String: ");
		scanf("%s", buffer);
