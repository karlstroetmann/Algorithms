...
\item Jeder Knoten besteht aus zwei Komponenten
  \begin{enumerate}
  \item Daten
  \item Zeiger auf nächsten Knoten
  \end{enumerate}
\end{enumerate}
...
\item Liste wird realisiert durch Zeiger auf Knoten
      ...
      \texttt{typedef NodePtr List} wird aus konzeptionellen Gründen
      eingeführt:  Unterscheidung Knoten $\longleftrightarrow$ Listen.
\item \texttt{Data} wird anderswo über \texttt{typedef} definiert
...
\item Anfügen eines Knotens an Listen--Anfang
...
Komplexitäten: Sei $n$ Länge von $l$
\begin{enumerate}
...
\footnotesize
\begin{enumerate}
\item Einfügen in sortierte Liste
...
\end{verbatim}
Aufwand \texttt{minSort}: $\Oh(n^2)$ falls $n$ Länge der Liste
...
Wichtig für Komplexität: Teile sind etwa gleich groß 
...
\item Rückgabe eines Ergebnisses: \hspace*{\fill} \texttt{return l}
\item Positionieren eines Arguments auf dem Stack für einen Funktions--Aufruf
...
\item 1 elementare Operation für Pointer--Zugriff \\[0.3cm]
      \hspace*{1.3cm} \texttt{ptr->nextPtr}
\item 1 elementare Operation für Zuweisung \\[0.3cm]
      \hspace*{1.3cm} \texttt{ptr->nextPtr = l}
\item 1 elementare Operation für Ergebnis--Rückgabe  \\[0.3cm]
...
\begin{center}
Anzahl der elementaren Operationen für \texttt{split}
\end{center}
\vspace*{0.5cm}

\footnotesize
$a_n$: Zahl elementaren Operationen bei Aufruf von \texttt{split} \\[0.3cm]
\hspace*{0.8cm} mit Liste der Länge $n$
...
\begin{center}
Lösung der Rekursions--Gleichungen
\end{center}
\vspace*{0.5cm}

\footnotesize
Wir betrachten die Fälle $n = 2*m$ und $n = 2* m + 1$.
\begin{enumerate}
\item $n = 2 * m$
      $$
      \begin{array}{lcl}
...
\hspace*{1.3cm}  $l_1$ Länge $n$ und $l_2$ Länge $m$ hat.
...
\item Fall: \texttt{l1 != 0} und \texttt{l2 == 0} 
      ...
      \end{enumerate}
\end{enumerate}
Definere für \emph{worst case} Abschätzung \\[0.3cm]
...
\end{enumerate}
Setze $f_k := d_{2^k}$. Das führt auf Rekursions--Gleichung \\[0.3cm]
...
\begin{center}
Lösung der Rekursions--Gleichung 
\end{center}
\vspace*{0.5cm}
...
\begin{center}
Lösung der Rekursions--Gleichung (Fortsetzung)
\end{center}
\vspace*{0.5cm}
...
\end{array}
$$
Lösung dies Gleichungs-Systems: \\[0.3cm]
...
Die Komplexität von \texttt{Merge--Sort} ist $\Oh(n * \log_2(n))$!
...
\footnotesize
Definition des Prädikats $\mathtt{leq}: \textsl{Data} \times \textsl{List} \rightarrow \mathbb{B}$
...
Distributivität von \texttt{leq} und \texttt{merge}: \\[0.3cm]
...
\footnotesize
Definition des Prädikats $\mathtt{ord}: \textsl{List} \rightarrow \mathbb{B}$
\begin{enumerate}
\item $\texttt{ord}([]) \leftrightarrow \mathtt{true}$
\item $\texttt{ord}([d|r]) \leftrightarrow \mathtt{leq}(d,r) \wedge \mathtt{ord}(r)$
\end{enumerate}
Distributivität von \texttt{ord} und \texttt{merge}: \\[0.3cm]
...
\hspace*{-2cm}
      ...
      &                 & (\mbox{wegen der Distributivität von \texttt{leq} und \texttt{merge}})  \\[0.3cm]
...
\item $\mathtt{length}(l) \geq 2$
      ...
      &                 & (\mbox{wegen der Distributivität von \texttt{ord} und \texttt{merge}}) \\[0.3cm]
