...
\footnotesize
Kaninchen--Farm: Business--Plan für Bunnies Incorporated
\begin{enumerate}
...
\end{verbatim}
Frage: Wieviel Additionen $a_n$ werden zur Berechnung von $\texttt{fib}(n)$ benötigt?
\begin{enumerate}
\item $a_1 = 0$.
\item $a_2 = 0$.
...
Lösung einer Rekurrenz--Gleichung durch \emph{Ansätze}:
\begin{enumerate}
\item $a_n = c$
...
\begin{center}
Lösung von $a_1 = a_2 = 0$, $a_n = 1 + a_{n-1} + a_{n-2}$
\end{center}
\vspace*{0.5cm}
...
\textbf{Lösung}: Merken der berechneten Werte!
...
Befehl ``\texttt{double fib[n]}'' durchgeführt werden.

$c$ hängt nicht von $n$ ab!

Speicherverbrauch: $8 * n + d$ Byte, wobei $d$ nicht von $n$ anhängt.
...
Zusätzlich sind Werte für $a_0$, $\cdots$, $a_k$ gegeben, die \\
...
Dieses Polynon heißt \emph{charakteristisches Polynom} der Rekurrenz--Gleichung.

Diese Polynom hat i.a.~$k+1$ verschiedene Lösungen: \\[0.3cm]
\hspace*{1.3cm}  $\lambda_0$, $\lambda_1$, $\cdots$, $\lambda_k$.

Allgemeine Lösung ist dann: \\[0.3cm]
\hspace*{1.3cm} $a_n = \alpha_0 \lambda_0^n + \cdots + \alpha_k \lambda_k^n$

Die Koeffizienten $\alpha_0$ bis $\alpha_k$ sind so zu wählen, dass die Anfangsbedingungen 
erfüllt sind.  Das liefert lineares Gleichungs--System für $\alpha_0$, $\cdots$, $\alpha_k$:
$$
\begin{array}{lcl}
...
Ansatz: $a_n = \lambda^n$ liefert \\[0.3cm]
 ...
 \end{array}
$$
Das gibt nur eine Lösung.  Neuer Ansatz: $a_n = n * \lambda^n$ mit $\lambda = 2$.
Das liefert
$$
\begin{array}{lcll}
...
\end{array}
$$
Also ist $n*2^n$ Lösung der obigen Rekurrenz--Gleichung.
...
Dann hat die allgemeine Lösung der Rekurrenz--Gleichung (*) die Form \\[0.3cm]
...
Gilt $\chi(\lambda) = 0$, so ist das charakteristisches Polynom für $\lambda$  \textbf{$m$--fach entartet}, 
falls es ein Polynom $\varphi(x)$ gibt mit 
\\[0.3cm]
\hspace*{1.3cm} $\chi(x) = (x - \lambda)^m * \varphi(x)$ \\[0.3cm]
Dann sind   für $i=1,\cdots,m$ die Folgen $a^{(i)}$ mit   \\[0.3cm]
\hspace*{1.3cm} $a^{(i)}_n = n^{i-1} * \lambda^n$ \\[0.3cm]
Lösungen der Rekurrenz--Gleichung (*).  Die restlichen Lösungen der Rekurrenz--Gleichung (*) können aus dem Polynom $\varphi$
gewonnen werden.
...
Lösungen der Rekurrenz--Gleichung sind also:
\begin{enumerate}
...
\footnotesize
Lösung der Anfangsbedingungen durch Linear--Kombination dieser Lösungen: \\[0.3cm]
\hspace*{1.3cm} 
$a_n = \alpha_1 * a^{(1)}_n + \alpha_2 * a^{(2)}_n + \alpha_3 * a^{(3)}_n$

Einsetzen für $n=1,2,3$ liefert lineares Gleichungs--System: 
$$
\begin{array}{lcl}
...
\begin{center}
Rückführung linearer \underline{inhomo}g\underline{ener} Rek.--Gl. auf \\[0.3cm]
...
Abziehen der ursprünglichen Gleichung liefert \\[0.3cm]
...
Also haben wir für Folge $(a_n)_n$ homogene Rek.--Gl. \\[0.3cm]
...
\footnotesize
Allgemeine Lösung der Rek.--Gl. (C): \\[0.3cm]
...
Also lautet Lösung der Rek.--Gl. \\[0.3cm]
...
\footnotesize
\begin{center}
Rückführung nicht--linearer Rekursions--Gleichungen auf lineare Rekursions--Gleichungen
\end{center}
\vspace*{0.5cm}
...
Das liefert Rekursions--Gleichungen für Folge $(b_k)_k$: \\[0.3cm]
...
\footnotesize
Allgemeine Lösung der Rek.--Gleichung (C): \\[0.3cm]
\hspace*{1.3cm} $b_k = \alpha * 1^k + \beta * 2^k$ 

Bestimmung von $\alpha$, $\beta$ über Anfangs--Bedingungen: \\[0.3cm]
