...
Die Funktion \textsl{cost} gibt an, wie viele Bits benoetigt werden, um mit dem gegebenen
...
Offenbar gibt es im Allgemeinen sehr viele Moeglichkeiten, die Knoten aus der Menge zu
...
\item Die Funktion \textsl{codingTree} wird mit einer Menge $m$ von Knoten aufgerufen,
      ...
      Prioritaets-Warteschlange gespart:  
      \begin{enumerate}
      ...
      \item Anstelle von $\textsl{insert}(m, p, v)$ koennen wir einfach
            \\[0.2cm]
            \hspace*{1.3cm}
            \texttt{$m$ += \{ [$p$, $v$] \};}
            \\[0.2cm]
            schreiben um das Element $v$ mit der Prioritaet $p$ in die
            Prioritaets-Warteschlange $m$ einzufuegen.
      \item Die Funktion $\textsl{remove}(m)$ realisieren wir durch den Aufruf
            \\[0.2cm]
            \hspace*{1.3cm}
            \texttt{$m$ -= \{ first($m$) \};}
            \\[0.2cm]
            denn $\textsl{remove}(m)$ soll ja das Element mit der hoechsten Prioritaet aus
            $m$ entfernen.
      \end{enumerate}
      Das Elegante an diesem Vorgehen ist, dass damit saemtliche Operationen des abstrakten
      Daten-Typs \textsl{PrioQueue} eine logarithmische Komplexitaet haben.  Das ist zwar
      im Falle der Operation $\textsl{top}(m)$ nicht optimal, aber fuer die Praxis voellig
      ...
      Komplexitaet, die dann auch im Falle der optimalen Implementierung  die gesamte
      Komplexitaet dominiert. 
\item Die \texttt{while}-Schleife in Zeile 2 veringert die Anzahl der Knoten in der Menge $m$
      ...
      \item Dazu werden mit Hilfe der Funktion $\textsl{first}()$ die
            beiden Knoten $a$ und $b$ berechnet, fuer die der Wert von $\textsl{count}()$ minimal
...
\section{Optimalitaet des Huffman'schen Kodierungsbaums}
...
\end{eqnarray}
gilt.  Wir zeigen nun, dass $n_2.\textsl{cost}() \leq n_1.\textsl{cost}()$ gilt.  Dazu geben wir zunaechst
$n_2.\textsl{cost}()$ an.
\begin{eqnarray*}
  n_2.\textsl{cost}() 
...
\end{eqnarray*}
Unter Beruecksichtigung der Gleichungen (\ref{eq:huffmannL1a}) bis (\ref{eq:huffmannL1e}) koennen wir dies
auch schreiben als
\begin{eqnarray*}  
  n_2.\textsl{cost}() 
...
erfuellt ist.  Da in dieser Ungleichung nur noch der Kodierungs-Baum $n_1$ vorkommt, vereinfachen wir die
Schreibweise und vereinbaren, dass wir einen Ausdruck der Form $n_1.\textsl{depth}(u)$ zu
$\textsl{depth}(u)$ abkuerzen.  Die letzte Ungleichung ist dann aequivalent zu der Ungleichung
\\[0.2cm]
\hspace*{0.8cm}
