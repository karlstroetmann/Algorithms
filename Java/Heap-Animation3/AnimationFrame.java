...
class TreePanel extends JPanel
{
    ...
    public void paintComponent(Graphics g) {
        ...
        paintNode(g2, root);
        Dimension dimension = 
            new Dimension((int) root.getWidth() + 20, (int) root.getHeight() + 20);
    ...
    private void paintNode(Graphics2D g2, HeapNode<Integer, Integer> node) {
        ...
        double value = ((Integer) node.getValue()).doubleValue() * 0.01;      
        double red   = min(2.0 * abs(value - 0.5), 1.0);
        double green = min(abs(cos(value * 0.5 * PI)),  1.0);
        double blue  = 0;
        ...
        g2.drawString(node.getKey().toString(), 
                      (int) (topX - 0.3 * HeapNode.sRadius), 
                      (int) (topY + 0.8 * HeapNode.sRadius));
		g2.drawString(node.getValue().toString(),
                      (int) (topX - 0.3 * HeapNode.sRadius), 
                      (int) (topY + 1.6 * HeapNode.sRadius));
		g2.drawLine((int) (topX - HeapNode.sRadius), 
					(int) (topY + HeapNode.sRadius),
					(int) (topX + HeapNode.sRadius),
					(int) (topY + HeapNode.sRadius));
...
public class AnimationFrame extends JFrame
{
    ...
    HeapTree<Integer, Integer>  mHeapTree;
    ...
    public AnimationFrame() {
        ...
        mHeapTree = new HeapTree<Integer, Integer>();
        for (int i = 1; i < 4; ++i) {
            mHeapTree.insert(i, i);
            mHeapTree.insert(100 - i, 100 - i);
        }
        ...
        TreePanel   panel      = new TreePanel(mHeapTree);
    ...
    // Listener for the change text field.  Chabges keys.
    class ChangeListener implements ActionListener
    {
        // Parse the number and insert or delete the key
        public void actionPerformed(ActionEvent e) {
            Integer value = Integer.parseInt(mInsertField.getText().trim());
            Integer key   = Integer.parseInt(mChangeField.getText().trim());
            mChangeField.setText(null);
            mInsertField.setText(null);
            mHeapTree.change(key, value);
            repaint();
        }
    }
}
