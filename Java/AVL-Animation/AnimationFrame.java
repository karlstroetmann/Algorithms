...
{
    private AVLTree<Integer, Integer> mBinaryTree;

    TreePanel(AVLTree<Integer, Integer> binaryTree) {
    ...
    public void paintComponent(Graphics g) {
        ...
        Node<Integer, Integer> root = mBinaryTree.mRoot;
        root.draw(0.0, 0.0);
        paintNode(g2, root);
        Dimension dimension = 
    ...
    private void paintNode(Graphics2D g2, Node<Integer, Integer> node) {
        ...
        double key   = node.getKey().doubleValue() * 0.01;      
...
//         for (int i = 0; i < 50; ++i) {
// 			avlTree.insert(random.nextInt(100), 0);
//         }
        avlTree = AVLTree.genSkewedTree(7, 0);
// 		for (int i = 0; i < 50; ++i) {
// 			avlTree.insert(i, 0);
// 		}
    ...
    class MyActionListener implements ActionListener
    {
        ...
        public void actionPerformed(ActionEvent e) {
            JTextField textField = (mInsert ? mInsertField : mDeleteField);
            Integer    key       = Integer.parseInt(textField.getText().trim());
            textField.setText(null);
            if (mInsert) {
                avlTree.insert(key, 0);
            } else {
                avlTree.delete(key);
            }
            repaint();
